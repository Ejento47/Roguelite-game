...
#render tiles  that are already SEEN and whatever withing FOV
    if fov_recompute:
   ...
   #draw entities in the list and render entities(this includes player as well)
    ...
    #printing of the messages in the message log
    y = 1
    for message in message_log.messages:
        libtcod.console_set_default_background(panel, message.color)
        libtcod.console_print_ex(panel, message_log.x, y, libtcod.BKGND_NONE, libtcod.LEFT, message.text)
        y+=1
    #rendering of the bar like HP
    render_bar(panel, 1, 1, bar_width, 'HP', player.fighter.hp, player.fighter.max_hp, libtcod.light_red, libtcod.darker_red)

    #render infor when mouse hovers arnd entities
...
def clear_all(con, entities):
    #loop that calls another function to clear entity after drawing them on screen
